...
void main() {
  TinniState makeState() {
    ...
    return state;
    testWidgets('top Mine is room-focused and separate from bottom profile Mine',
      (tester) async {
    final state = makeState();
    state.discovery.createRoom(
      title: 'My Test Room',
      country: 'IN',
    );
  ...
  testWidgets('top tabs are tappable and PageView swipes between sections',
      (tester) async {
    final state = makeState();
    await tester.pumpWidget(TinniStarApp(state: state));
    await tester.pumpAndSettle();
