  ...
  Future<void> _chooseDp() async {
    final source = await showModalBottomSheet<ImageSource?>(
      ...
      builder: (sheetContext) => SafeArea(
        child: Wrap(children: [
          ListTile(leading: const Icon(Icons.photo_library_rounded), title: const Text('Gallery'), onTap: () => Navigator.pop(sheetContext, ImageSource.gallery)),
          ListTile(leading: const Icon(Icons.camera_alt_rounded), title: const Text('Camera'), onTap: () => Navigator.pop(sheetContext, ImageSource.camera)),
          ListTile(
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        const Center(child: Text('Tap Room DP • Gallery / Camera / Emoji')),
        const SizedBox(height: 14),
...
class _RoomV06State extends State<RoomV06> {
  ...
  void initState() {
    ...
    if (seats.length > 2) { seats[2] = 'Aisha'; mutedSeats.add(2); }
    if (seats.length > 6) seats[6] = 'Sam';
    for (final i in [8, 14, 24]) { if (i < seats.length) lockedSeats.add(i); }
  }
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                ...
                _BottomTool(micOn ? Icons.mic_rounded : Icons.mic_off_rounded, micOn ? 'Mic On' : 'Mic Off', () { setState(() => micOn = !micOn); }),
                _BottomTool(Icons.card_giftcard_rounded, 'Gift', _gift),
                _BottomTool(Icons.person_add_alt_1_rounded, 'Invite', () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(inviteMode ? 'Invite request created' : 'Room link ready to share')))),
              ]),
            ),
          ]),
        ),
    ...
    showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(labelText: 'LP amount (min 6,000)', border: OutlineInputBorder()),
        ),
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              if (value < 6000) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lucky Bag minimum is 6,000 LP')));
                return;
              }
  ...
  void _playGame(String name) {
    final random = Random();
    final result = name == 'Dice' ? '${random.nextInt(6) + 1}' : ['10x', '2x', 'Try Again', '5x'][random.nextInt(4)];
    setState(() => chat.add('Game: $name result → $result'));
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$name result: $result')));
  }
  ...
  Future<void> _changeRoomDp() async {
    final source = await showModalBottomSheet<ImageSource?>(
      ...
      builder: (sheetContext) => SafeArea(
        child: Wrap(children: [
          ListTile(leading: const Icon(Icons.photo_library_rounded), title: const Text('Gallery'), onTap: () => Navigator.pop(sheetContext, ImageSource.gallery)),
          ListTile(leading: const Icon(Icons.camera_alt_rounded), title: const Text('Camera'), onTap: () => Navigator.pop(sheetContext, ImageSource.camera)),
          ListTile(leading: const Icon(Icons.emoji_emotions_rounded), title: const Text('Emoji DP'), onTap: () {
            setState(() {
              widget.room.dpPath = null;
              widget.room.dp = widget.room.dp == '🎧' ? '👑' : widget.room.dp == '👑' ? '🌙' : '🎧';
            });
            Navigator.pop(sheetContext);
          }),
        ]),
    ...
    if (source == null) return;
    final file = await picker.pickImage(source: source, imageQuality: 80, maxWidth: 1200);
    if (file != null && mounted) setState(() => widget.room.dpPath = file.path);
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => SafeArea(
        child: GridView.builder(
          ...
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 2, crossAxisSpacing: 10, mainAxisSpacing: 10),
          itemBuilder: (_, i) => InkWell(
            onTap: () { setState(() => backgroundIndex = i); Navigator.pop(sheetContext); },
            child: Container(
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), gradient: LinearGradient(colors: roomBackgrounds[i]), border: Border.all(color: Colors.white70)),
  ...
  void _admins() {
    final candidates = seats.whereType<String>().where((m) => m != 'Owner' && m != 'You').toList();
    showModalBottomSheet<void>(
      context: context,
            ...
            children: [
              ...
              for (final m in candidates)
                SwitchListTile(
                  ...
                  onChanged: (v) { setState(() => v ? admins.add(m) : admins.remove(m)); setLocal(() {}); },
                ),
            ],
          ),
        ),
        ...
        builder: (context, setLocal) => SafeArea(
          child: ListView(
            ...
            children: [
              ...
              for (final m in blocked.toList())
                ListTile(title: Text(m), trailing: TextButton(onPressed: () { setState(() => blocked.remove(m)); setLocal(() {}); }, child: const Text('Unblock'))),
              const Divider(),
              for (final m in candidates.where((m) => !blocked.contains(m)))
                ListTile(title: Text(m), trailing: TextButton(onPressed: () {
                  setState(() { blocked.add(m); final i = seats.indexOf(m); if (i >= 0) seats[i] = null; });
                  setLocal(() {});
        ...
        child: ListView(
          ...
          children: [
            ...
            for (final gift in const ['Rose 🌹', 'Crown 👑', 'Rocket 🚀'])
              ListTile(title: Text(gift), onTap: () { setState(() => chat.add('You sent $gift')); Navigator.pop(sheetContext); }),
          ],
        ),
      ),
    );
...
class _GameCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(children: [Icon(icon, size: 40), const SizedBox(height: 8), Text(label, style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 8), FilledButton(onPressed: onPlay, child: const Text('Play'))])));
}
).hasMatch(cleanPin)) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Password must be exactly 6 digits')));
              return;
            }
            Navigator.pop(
              context,
